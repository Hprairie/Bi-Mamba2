...
def naive_fwd(x, dt, A, B, C, D, delta_bias, z, chunk_size, delta_softplus):
    ...
    _, out_x2, _, _, _, _ = _mamba_chunk_scan_combined_fwd(
            x=x.flip([1]), dt=dt.flip([1]), A=A, B=B.flip([1]), C=C.flip([1]), D=D, dt_bias=delta_bias, z=z.flip([1]), chunk_size=chunk_size, dt_softplus=delta_softplus,
            )
...
def benchmark(seqlen, provider):
    ...
    if provider == "Naive Mamba2":
        ms, min_ms, max_ms = triton.testing.do_bench(lambda: naive_fwd(x, dt, A, B, C, D, delta_bias, z, chunk_size, delta_softplus), quantiles=quantiles, rep=2000, warmup=500)
